...
For large transformations, do not necessarily run `rebuildRows()` synchronously on the main actor.
...
Formatting is easy to hide inside views because it looks like presentation work. In large or frequently updated views, it can still be repeated CPU and allocation work.
...
- formatted strings in render models;
- cached formatter instances when using Foundation formatters;
...
1. 1Read or receive raw data.
2. 2Transform raw data into render models off the main actor when safe.
3. 3Apply one compact final state update on the main actor.
4. 4Render prepared values.
...
- Are cache invalidation rules clear for locale, currency, calendar, and settings?
...
- Do not rebuild all render models when only one row changed.
...
- Do not cache formatted values without considering locale, calendar, currency, time zone, and user settings.
