...
1. 1Confirm the scenario is launch-related, not only resume, scene restoration, or foreground refresh.
2. 2Identify which lifecycle owner runs the work: app delegate, scene delegate, root UI factory, root coordinator, or first-screen setup.
3. 3Separate app-wide process bootstrap from scene-specific UI construction.
4. 4Classify each startup task by necessity: first frame, first interaction, soon after launch, feature-lazy, or background maintenance.
5. 5Look for synchronous or hidden eager work on the main thread.
6. 6Check whether routing, notification, universal link, shortcut, or restoration input forces early decisions.
7. 7Preserve correctness requirements such as auth, privacy lock, security, crash reporting, state restoration, multi-window behavior, and compliance.
8. 8Recommend the smallest safe change and a concrete validation path.
## App Delegate Review
...
Recommended model:
1. 1Parse launch input cheaply.
2. 2Store a minimal pending route or initial intent.
3. 3Show a safe root UI.
4. 4Resolve expensive authorization, data, or module requirements after the root UI exists.
5. 5Complete deep-link or notification navigation only when the required state is available.
Review for:
